...
async function _getQwenHeadersInternal(forceNew = false): Promise<{
  ...
  if (!isOnQwen || forceNew || isOnSpecificChat) {
    console.log(
      `[Playwright] Navigating to Qwen home... (Current: ${currentUrl})`,
    );
    await activePage.goto('https://chat.qwen.ai/', {
      waitUntil: 'domcontentloaded',
    });
  ...
  await activePage
    .waitForSelector(inputSelector, { timeout: 30000 })
    .catch(() => {
      console.error(
        '[Playwright] Chat input not found. Current URL:',
        activePage?.url(),
      );
  ...
  return new Promise((resolve, reject) => {
    ...
    activePage
      ...
      .then(async () => {
        ...
        if (!clicked) {
          console.log(
            '[Playwright] No send button found/clicked, fallback to Enter...',
          );
          await activePage?.focus(inputSelector);
          await activePage?.keyboard.press('Enter');
        }
