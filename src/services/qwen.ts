...
const sessionStates: Record<string, string | null> =
  (globalThis as { _sessionStates?: Record<string, string | null> })
    ._sessionStates || {};
(
  globalThis as { _sessionStates?: Record<string, string | null> }
...
let cachedModels: QwenModel[] | null = null;
let lastModelsFetch = 0;
let nativeToolsDisabled = false;
let disablingNativeToolsInProgress = false;

export function getModel(modelId: string): QwenModel | undefined {
  const modelsList = cachedModels as QwenModel[];
  const found = modelsList.find((m) => m.id === modelId);
  return found; // Retorna 131072 como fallback geral
}
...
export async function fetchQwenModels(): Promise<QwenModel[]> {
  const now = Date.now();
  ...
  if (json.data && Array.isArray(json.data)) {
    const models = json.data.map(
      (m: {
        ...
        info?: {
          created_at?: number;
          meta?: { max_context_length?: number };
        };
      }) => ({
        id: m.id,
        object: 'model',
        ...
        max_context_length: m.info?.meta?.max_context_length || 131072,
      }),
    );
