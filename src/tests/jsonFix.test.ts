...
try {
  const result = robustParseJSON(problematicString) as {
    name: string;
    arguments: {
      suggest: string;
      actions: {
...
try {
  const result = robustParseJSON(missingBraces) as {
    name: string;
    arguments: {
      foo: string;
    };
...
try {
  ...
  const result = robustParseJSON(literalNewline) as {
    name: string;
    msg: string;
  };
  console.log('Successfully parsed:', JSON.stringify(result, null, 2));
...
try {
  const result = robustParseJSON(crazyCase) as {
    name: string;
    arguments: {
      suggest: string;
      actions: {
...
try {
  const result = robustParseJSON(invalidEscapes) as {
    path: string;
  };
  console.log('Successfully parsed:', JSON.stringify(result, null, 2));
  if (
...
const doubleKey =
  '{"name": "name": "create_file", "arguments": {"path": "b.txt"}}';
console.log('\nTesting double key hallucination...');
try {
  const result = robustParseJSON(doubleKey) as {
    name: string;
    arguments: {
      path: string;
    };
...
const unquotedArgs =
  '{"name":"Read",arguments:{"file_path":"test.ts","limit":100}}';
console.log('\nTesting unquoted arguments key...');
try {
  const result = robustParseJSON(unquotedArgs) as {
    name: string;
    arguments: {
      file_path: string;
      limit: number;
